...
var FlagContribBasica = Salario > (7 * URP) ? false : true;
//var DtPercPatroc = DtLimitAdesao;                    //Data de comparação com a data efetiva do participante no plano GebsaPrev, percentual de patroc
//var secContribtblCtr1 = '<i id="secContribtblCtr1" class="fa fa-question-circle-o fa-2" style="font-size:15px; background-color: white;border-radius: 60%;" aria-hidden="true" data-toggle="tooltip" data-placement="bottom"  title="A contribuição será igual à parcela do salário que exceder 15 UPs multiplicado por 7%."></i>';
...
function Monta_Contrib(){
    ...
    var fnc4 = '<select class="form-control input-sm" id="fnc4" size="1" style="width:95px;"></select>';
    ...
    var ctr1fd = new tbfld("Suplementar Participante",  fnc1, "", "", fnc3);

    //config grupo contribuição 2
    var ctr2hd = new tbfld("", "Percentual Contrib.", "", "", "Contrib. Simulada");
    var ctr2fd = new tbfld("Básica Patrocinadora", fnc2, "", "", fnc10);     

    //config grupo contribuição 2
    var ctr3hd = new tbfld("", "Percentual Contrib.", "", "", "Contrib. Simulada");
    var ctr3fd = new tbfld("Suplementar Patrocinadora", fnc4, "", "", fnc11);     
    
    //config grupo contribuição 3
    var ctr4hd = new tbfld("", fnc17, fnc24 + fnc25, "Periodicidade", "Contrib. Simulada");
    var ctr4fd = new tbfld(fnc19, fnc20, fnc26 + fnc21, fnc22, fnc23);

    //config grupo contribuição 4    
    var ctr5hd = new tbfld("", fnc18, fnc13 + fnc14, "Periodicidade", "Contrib. Simulada");
    var ctr5fd = new tbfld(fnc15, fnc5, fnc6 + fnc9, fnc7, fnc8);
...
function Contrib_Control(){
    ...
    //Adiciona 0% para começar o for no 1 
    $('#fnc6').append('<option value=' + x / 100 + '>' + $.formatNumber(0, {format: "#,##0.0", locale: "br"}) + '%</option>');   
    ...
    if (FlagContribBasica){              
      ...
      $('#fnc1').prop("selectedIndex", 10);  
      $('#fnc2').prop("selectedIndex", 1);
...
//----------------------------------------------------- Inicializa Contribuições ---------------------------------------------------------                 
    $('#fnc3').val("R$ " + $.formatNumber((Salario - 7 * URP) * $('#fnc1').val(), {format: "#,##0.00", locale: "br"}));
    ContribCalc[0][0] = (Salario - 7 * URP) * $('#fnc1').val();
    ContribCalc[0][1] = 0;
    ContribCalc[0][2] = 1;
    ContribCalc[0][3] = 0;                  

    $('#fnc10').val("R$ " + $.formatNumber((Salario - 7 * URP) * $('#fnc2').val(), {format: "#,##0.00", locale: "br"}));
    ContribCalc[1][0] = (Salario - 7 * URP) * $('#fnc2').val();
    ContribCalc[1][1] = 1;
    ContribCalc[1][2] = 1;
    ContribCalc[1][3] = 0;
    
    $('#fnc11').val("R$ " + $.formatNumber(ContribCalc[0][0], {format: "#,##0.00", locale: "br"}));
    ...
    ContribCalc[3][1] = 0;  
    ContribCalc[3][2] = 1; 
    ContribCalc[3][3] = ($('#fnc22').val() == "Mensal") ? 0 : ($('#fnc22').val() == "Anual") ? 1 : 2; //Periodicidade da contribuição - 0: mensal, 1: anual, 2: único
...
//------------------------------------------------------- Eventos Change ---------------------------------------------------------           
    ...
    $('#fnc7').change(function() {
       ContribCalc[4][3] = ($('#fnc7').val() == "Mensal") ? 0 : ($('#fnc7').val() == "Anual") ? 1 : 2;
       /*
       if (status == 9){
        BenefBar(BenefCheckOpt);
       } 
    ...
    /* if ((Salario * $('#fnc6').val() + ContribCalc[0][0] + ContribCalc[2][0]) > Salario) {                  
        ...
        ContribCalc[4][0] = Salario * $('#fnc6').val();
        ContribCalc[4][1] = 0;
        ContribCalc[4][2] = 1;
     /* }; */                   
    });
...
function catchContribuicao(){  //Captura pagamento pela conta suplementar, Voluntaria
  ...
  do {
    ...
    if ((movto.id_conta == 1204) && movto.conta_resp == 0 && ctrCatchVol == 0 && movto.competencia == DtBase){ //Pega a última Contrib Voluntaria      
      ctrVolunt =  flagDtBaseDezembro ? (parseInt(movto.ctr_rent/2 * 100)/100) : movto.ctr_rent;
      ctrCatchVol++;
    }
    i--;
  } while(i >= 0)
...
function percPatrocinadora(){ //Descobre o percentual de contrib da patrocinadora
   ...
   // Testar o cálculo de idade dessa linha
