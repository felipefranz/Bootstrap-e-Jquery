...
//------------------------------------------------------- Eventos Change --------------------------------------------------------- 
    ...
    $('#fnc21').change(function () {
      ...
      if (isNaN($('#fnc21').val())){
      ...
      } else if ($('#fnc21').val() < ContribCalc[0][0]) {
          $('#fnc12').html('<strong>Atenção!</strong> Contribuição eventual não pode ser inferior a R$ '+ $.formatNumber(ContribCalc[0][0], {format: "#,##0.00", locale: "br"})+'.');  
          $('#fnc12').fadeIn();
          $('#fnc21').val("");        
          $('#fnc23').val("R$ " + $.formatNumber(0, {format: "#,##0.00", locale: "br"}));
          ContribCalc[2][0] = 0;
      ...
      } else if ((parseFloat($('#fnc21').val()) + ContribCalc[0][0] + ContribCalc[3][0]) > (Salario * 0.3)) {                  
          $('#fnc12').html('<strong>Atenção!</strong> Valor de contribuição eventual acima do permitido, digite um novo valor.');
          $('#fnc17').html('A Contribuição Eventual Participante está sujeita à avaliação de margem salarial disponível.');          
          $('#fnc12').fadeIn();
          $('#fnc17').fadeIn();          
          $('#fnc21').val("");        
          $('#fnc23').val("R$ " + $.formatNumber(0, {format: "#,##0.00", locale: "br"}));
        ...
        ContribCalc[2][0] = parseFloat($('#fnc21').val());
        ContribCalc[2][1] = 0;
        ContribCalc[2][2] = 1;
