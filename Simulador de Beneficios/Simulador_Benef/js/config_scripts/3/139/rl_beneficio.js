...
function rl_sliderPerc(){
     if (DtSaldoDIB >= DireitoAposent()){  
        if (SlideValSaque == 0 && $("#wdwperc").val().replace('%','') != 0){
            //Apresenta mensagem
            $('#secSaque_msg2').html('<strong>Atenção!</strong> A opção pelo saque de até 25% à vista, somente será válida quando o valor de <strong>Benefício Inicial Bruto</strong> for superior a R$'+$.formatNumber((5*URAG),{format: "#,##0.00", locale: "br"})+'<br/> (5 URAG).');  
...
function validaSaque(PercSaque){
  ...
  if ($("#perc").is(":checked")) { //Percentual - P                
       (SP - (SP * PS)) * (ValPerc/100) <= (5 * URAG) ? PS = 0 : PS = PS;
            
  }  else if ($("#prazo").is(":checked")){ //Prazo Certo - PC            
       (SP - (SP * PS)) / (ValPrazo * Nper) <= (5 * URAG) ? PS = 0 : PS = PS;   
                    
  }  else { //Renda Certa - RC                        
       (SP - (SP * PS)) * ValRenda <= (5 * URAG) ? PS = 0 : PS = PS;                       
  }
    return PS;  
}
