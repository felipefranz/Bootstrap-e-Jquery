...
function Monta_Contrib(){
    ...
    var fnc1 ;
    if (Salario <= URP){     
	     fnc1 = '<input type="text" class="form-control input-sm" id="fnc1" size="1" style="width:80px;" onclick="clearInput(this)">';
    } else {
	     fnc1 = '<select class="form-control input-sm" id="fnc1" size="1" style="width:80px;"></select>';
    }
...
function Contrib_Control(){
    ...
    if (Salario <= URP){
       percPart = (UltContribPart/Salario) * 100; //Calcula 1º percentual de participante  
       $('#fnc1').val(''+$.formatNumber(percPart, {format: "#,##0.00", locale: "br"})+'%');     
    } else {
    for (x = 0.5; x <= 5; x += 0.1){
	...
	$('#fnc1').val(''+$.formatNumber((UltContribPart/Salario), {format: "#,###0.000", locale: "en"}));	  
      ...
      ContribCalc[2][1] = 0;
      ContribCalc[2][2] = 1;
      ContribCalc[2][3] = ContribCalc[2][3] = ($('#fnc22').val() == "Mensal") ? 0 : ($('#fnc22').val() == "Anual") ? 1 : 2; //Periodicidade da contribuição - 0: mensal, 1: anual, 2: único
    ...
    $('#fnc1').change(function() {
      ...
      if (Salario <= URP){ 
         ...
         if (isNaN($('#fnc1').val())){
         ...
         } if ($('#fnc1').val() > 100 || $('#fnc1').val() < 0) {
            $('#fnc27').html('<strong>Atenção!</strong> Digitar um valor entre 0% e 100%.');  
            $('#fnc27').fadeIn();
            $('#fnc1').val("");        
            $('#fnc3').val("R$ " + $.formatNumber(0, {format: "#,##0.00", locale: "br"})); 
            ContribCalc[0][0] = 0;
            ...
            ContribCalc[0][2] = 1;                                  
            setTimeout(function(){$('#fnc27').fadeOut();}, 4000);
           } if( (Salario * ($('#fnc1').val()/100)) < URAG ){                 
              $('#fnc28').html('<strong>Atenção!</strong> Valor de contribuição deve ser maior que R$'+$.formatNumber((URAG-0.01), {format: "#,##0.00", locale: "br"})+'.');  
              $('#fnc28').fadeIn();
              $('#fnc1').val(''+$.formatNumber(0, {format: "#,##0.00", locale: "br"})+'%');        
              $('#fnc3').val("R$ " + $.formatNumber(0, {format: "#,##0.00", locale: "br"})); 
              ContribCalc[0][0] = 0;
              ContribCalc[0][1] = 0;
              ContribCalc[0][2] = 1;                                  
         ...
         if( (Salario * $('#fnc1').val()) < URAG ){
         
              $('#fnc28').html('<strong>Atenção!</strong> Valor de contribuição deve ser maior que R$'+$.formatNumber((URAG-0.01), {format: "#,##0.00", locale: "br"})+'.');  
              $('#fnc28').fadeIn();
	      $('#fnc1').val(''+$.formatNumber(0, {format: "#,###0.000", locale: "en"}));        
              $('#fnc3').val("R$ " + $.formatNumber(0, {format: "#,##0.00", locale: "br"}));               
              ContribCalc[0][0] = 0;
              ContribCalc[0][1] = 0;
              ContribCalc[0][2] = 1;                                  
