...
var contribEspecPatroc = 0;  //Contribuição Específica Patrocinado
...
function Monta_Contrib(){
    ...
    //var fnc17 = '<span id="fnc17">Básica Patrocinadora<span>';
...
//----------------------------------------------------- Inicializa Contribuições ---------------------------------------------------------  
    ...
    //Inicializa Contribuições
      ...
      contribEspecPatroc = Salario * indiceAtuarial(); //Descobre Contribuição específica conforme indice Atuarial
      ...
      $('#fnc10').val("R$ " + $.formatNumber(ContribCalc[0][0] - contribEspecPatroc, {format: "#,##0.00", locale: "br"}));
      ContribCalc[1][0] = ContribCalc[0][0] - contribEspecPatroc;
      ContribCalc[1][1] = 1;
      ContribCalc[1][2] = 1;
      ContribCalc[1][3] = 0;

      } else if (Salario > (14 * URP) && Salario <= (33 * URP)){
      
      $('#fnc1').prop("selectedIndex", 9);
...
//------------------------------------------------------- Eventos Change --------------------------------------------------------- 
    ...
    $('#fnc6').change(function() {
      ...
      ContribCalc[3][0] = ContribCalc[0][0] * $('#fnc6').val();
      ContribCalc[3][1] = 0;
      ContribCalc[3][2] = 1;
    });
...
var indiceAtuarial = function(){ //Descobre Índice Atuarial conforme última contribuição patrocinadora e participante
    
    var diferenca = 0;
    var contribPart = 0;        
    var contribPatroc = 0;       
    
    for (i in TextMovto.movimentacoes){  //Coleta o valor da última contribuição patrocinadora e participante
        var movto = TextMovto.movimentacoes[i];
        if (movto.id_conta == 1202 && movto.conta_resp == 0 && movto.ctr_rent > 0){
          contribPart =  movto.ctr_rent;         
        }
        if (movto.id_conta == 1301 && movto.conta_resp == 1 && movto.ctr_rent > 0){
          contribPatroc = movto.ctr_rent;          
        }          
     }
     
    diferenca = contribPart - contribPatroc;
    indice = ""+(diferenca / Salario);  
    indice = indice.substr(0, 6);  //Separa as casas depois da vírgula #.####
    indice = indice * 1;           //transforma em número
