...
function Monta_Contrib(){
    ...
    var ctr1fd = new tbfld("Contribuição Programada", fnc1, fnc2, "", fnc3);
...
//----------------------------------------------------- Inicializa Contribuições ---------------------------------------------------------
    ...
    if (Salario > 10 * URP){
      ...
      $('#fnc1').val() == 0.02 && $('#fnc2').val() == 0.07 ?  $('#fnc6').removeAttr("disabled", "disabled") :  $('#fnc6').attr("disabled", "disabled");        
    }else{
      $('#fnc2').prop("selectedIndex", 0);
      $('#fnc2').attr("disabled", "disabled");       
      $('#fnc1').val() == 0.02 && $('#fnc2').val() == 0.00 ?  $('#fnc6').removeAttr("disabled", "disabled") :  $('#fnc6').attr("disabled", "disabled");        
    };   
    
    if ($('#fnc1').val() == 0.02 && $('#fnc2').val() == 0.07){
        $('#fnc6').removeAttr("disabled", "disabled");
      } else {
...
//------------------------------------------------------- Eventos Change ---------------------------------------------------------
    ...
    $('#fnc6').change(function() {
      if ((Salario * $('#fnc6').val()) + ContribCalc[0][0] > Salario * 0.3){
          $('#fnc27').html('<strong>Atenção!</strong> Valor de contribuição adicional acima do permitido, selecione um novo percentual.');
          $('#fnc12').html('A Contribuição Adicional está sujeita à avaliação de margem salarial disponível.');          
          $('#fnc27').fadeIn();
          $('#fnc12').fadeIn();
          $('#fnc6').prop("selectedIndex", 0)
          $('#fnc6').focus();                          
