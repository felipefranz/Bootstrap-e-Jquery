...
//var minCtr = 0;
var percPatrocBasica = 0; //percentual de contribuição patrocinadora
var percPatrocGeral = 0; //percentual de contribuição patrocinadora
...
function Monta_Contrib(){
    ...
    var fnc4 = '<input type="text" class="form-control input-sm" id="fnc4" style="width:80px;" readonly>';
    ...
    var ctr1fd = new tbfld("Geral Patrocinadora", fnc1, "", "", fnc3);

    //config grupo contribui&ccedil;&atilde;o 2
    var ctr2hd = new tbfld("", "Percentual Contrib.", "Fator", "", "Contrib. Simulada");
    var ctr2fd = new tbfld(" Básica Patrocinadora", fnc2, fnc4, "", fnc10);
...
function Contrib_Control(){
    ...
    */
    ...
    percPatrocGeral = percPatrocinadora()[0];
    if (Salario >= URP) {
      percPatrocBasica = percPatrocinadora()[1];
      fatorPatrocBasica = percPatrocinadora()[2];
    }

    $('#fnc1').val("" + $.formatNumber(percPatrocGeral * 100, {format: "#,##0.00", locale: "br"}) + "%");
    $('#fnc1').attr("disabled", "disabled");

    if (Salario >= URP) {
      $('#fnc2').val("" + $.formatNumber(percPatrocBasica * 100, {format: "#,##0.00", locale: "br"}) + "%");
      $('#fnc4').val("" + $.formatNumber(fatorPatrocBasica, {format: "#,##0.00", locale: "br"}));
...
//----------------------------------------------------- Inicializa Contribuições ---------------------------------------------------------
    ...
    $('#fnc3').val("R$ " + $.formatNumber(Salario * percPatrocGeral, {format: "#,##0.00", locale: "br"}));
    ContribCalc[0][0] = Salario * percPatrocGeral;
    ContribCalc[0][1] = 1;
    ContribCalc[0][2] = 1;
    ContribCalc[0][3] = 0;

    if (Salario >= URP) {
      $('#fnc10').val("R$ " + $.formatNumber(((Salario - URP) * percPatrocBasica) * fatorPatrocBasica, {format: "#,##0.00", locale: "br"}));
      ContribCalc[1][0] = ((Salario - URP) * percPatrocBasica) * fatorPatrocBasica;
      ContribCalc[1][1] = 1;
      ContribCalc[1][2] = 1;
      ContribCalc[1][3] = 0;
    }
...
function percPatrocinadora (){
   ...
   var percePatrocGeral = 0.01; // 1% sobre o salário do participante
   var fatorPatroc = 0;  // fator calculado sobre a Básica
   var percePatrocBasica = 0.03; // 3% sobre o salário do participante
  ...
  return [percePatrocGeral, percePatrocBasica, fatorPatroc];
}
