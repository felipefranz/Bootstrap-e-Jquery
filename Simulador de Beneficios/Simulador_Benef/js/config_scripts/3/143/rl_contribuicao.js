...
function Monta_Contrib(){
    ...
    if (Salario >= (15 * URP)) {//Participante com salario acima de 15 URP 
...
//----------------------------------------------------- Inicializa Contribuições ---------------------------------------------------------  
    ...
    //Inicializa Contribuições
      ...
      $('#fnc3').val("R$ " + $.formatNumber((Salario -(8 * URP)) * $('#fnc1').val(), {format: "#,##0.00", locale: "br"})); 
      ContribCalc[0][0] = (Salario -(8 * URP)) * $('#fnc1').val();
      ContribCalc[0][1] = 0;
      ContribCalc[0][2] = 1;
      ContribCalc[0][3] = 0;
      ...
      $('#fnc23').val("R$ " + $.formatNumber(0, {format: "#,##0.00", locale: "br"}));
      ContribCalc[0][0] = 0; //valor da contribuição
      ContribCalc[0][1] = 0; //responsável pela contribuição - Patroc ou Partic
      ContribCalc[0][2] = 1; //Perfil da contribuição
      ContribCalc[0][3] = ContribCalc[0][3] = ($('#fnc22').val() == "Mensal") ? 0 : ($('#fnc22').val() == "Anual") ? 1 : 2; //Periodicidade da contribuição - 0: mensal, 1: anual, 2: único 
...
//------------------------------------------------------- Eventos Change --------------------------------------------------------- 
    ...
    $('#fnc22').change(function() {
      if (Salario >= (15 * URP)){  
           ...
           ContribCalc[0][3] = ($('#fnc22').val() == "Mensal") ? 0 : ($('#fnc22').val() == "Anual") ? 1 : 2;
        };
       /*if (status == 9){
        BenefBar(BenefCheckOpt);
       }*/ 
    ...
    $('#fnc26').change(function() {
      ...
      ContribCalc[0][0] = Salario * $('#fnc26').val();
      ContribCalc[0][1] = 0;
      ContribCalc[0][2] = 1;      
      }
