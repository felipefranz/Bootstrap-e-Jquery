...
//----------------------------------------------------------------------------------
//alterações feitas por Edwar Fonseca (EBF) em 27/jan/2020
// - linha 131 e 133 - Function ContribControl, comentar formas de periodicidade
// - linha 175 e 198 - aprimorar forma de calculo para valor menor que 10 URP
// - linha 301 - adicionada clausula para limite 100% patroc apos elegibilidade
//----------------------------------------------------------------------------------
...
function Monta_Contrib(){
    ...
    var ctr1hd = new tbfld("", "1ª Faixa", "2ª Faixa", "", "Contrib. Simulada");
    var ctr1fd = new tbfld("Contribui&ccedil;&atilde;o Participante", fnc1, fnc2, "", fnc3);

    //config grupo contribui&ccedil;&atilde;o 2
    var ctr2hd = new tbfld("", "", "", "", "Contrib. Simulada");
    var ctr2fd = new tbfld("Contribui&ccedil;&atilde;o Patrocinadora", "", "", "", fnc10);
...
function Contrib_Control(){
    ...
    $('#fnc7').append('<option value="Unico">Único</option>');   //comentado por edwar em 27/jan/2020
    $('#fnc7').append('<option value="Mensal">Mensal</option>');
    $('#fnc7').append('<option value="Anual">Anual</option>');   //comentado por edwar em 27/jan/2020
    ...
    if (Salario > 10 * URP){
      ...
      $('#fnc1').attr("disabled", "disabled"); //multipensions o % da primeira faixa sempre será 1%   
    };       
...
//EBF: 27/jan/2020:     $('#fnc3').val("R$ " + $.formatNumber((Salario - 10 * URP) * $('#fnc2').val() + (Salario >= (10 * URP) ? (10 * URP) : Salario) * $('#fnc1').val(), {format: "#,##0.00", locale: "br"}));
//EBF: 27/jan/2020: contribuicao minima deve ser sempre 10% da URP - ajustado codigo abaixo:
    $('#fnc3').val("R$ " + $.formatNumber((Salario - 10 * URP) * $('#fnc2').val() + (Salario >= (10 * URP) ? (10 * URP) : (10 * URP)) * $('#fnc1').val(), {format: "#,##0.00", locale: "br"}));
//EBF: 27/jan/2020:     ContribCalc[0][0] = (Salario - 10 * URP) * $('#fnc2').val() + (Salario >= (10 * URP) ? (10 * URP) : Salario) * $('#fnc1').val();
//EBF: 27/jan/2020: contribuicao minima deve ser sempre 10% da URP - ajustado codigo abaixo:
    ContribCalc[0][0] = (Salario - 10 * URP) * $('#fnc2').val() + (Salario >= (10 * URP) ? (10 * URP) : (10 * URP)) * $('#fnc1').val();
    ContribCalc[0][1] = 0;
    ContribCalc[0][2] = 1;
    ContribCalc[0][3] = 0;
...
//EBF: 27/jan/2020: incluido o segundo case para casos onde já é elegível, o percentual patroc deve ficar em 100%

   switch (true){   //Percentual de contribuição patrocinadora
      case (tempo_servi < 10 || DtSaldoDIB >= DireitoAposent()):
        percePatroc = 1;
        break;
     // case (tempo_servi >= 10 && DataDif(Ncmto,today,0,2) >= 55):
       // percePatroc = 1;
       // break;
      case (tempo_servi >= 10 && tempo_servi < 20):
        percePatroc = 1.5;
        break;    
      case (tempo_servi >= 20):
        percePatroc = 2;
