...
var NovoRegulamento = null;          //Participante inscrito no plano até a data de aprovação do novo regulamento 10/12/2009 (data efetiva do novo regulamento) 0 = Sim, 1 = Não
...
function Monta_Contrib(){
    ...
    var fnc1 = '<input type="text" class="form-control input-sm" id="fnc1" size="1" style="width:80px;" onclick="clearInput(this)">';
    var fnc2 = '<input type="text" class="form-control input-sm" id="fnc2" size="1" style="width:80px;" onclick="clearInput(this)">';
...
function Contrib_Control(){    
    ...
    $('#fnc2').val($.formatNumber(0/100, {format: "#,##0.00", locale: "br"})+"%");
...
//----------------------------------------------------- Inicializa Contribuições ---------------------------------------------------------  
    ...
    //Inicializa Contribuições
      ...
      NovoRegulamento = novoRegulamento();
      ...
      if (Salario < (9.5 * URP) && NovoRegulamento == 0) {
                           
        $('#fnc1').val(2); //Pencertual mínimo para participante neste status
        ...
        $('#fnc1').val(''+$.formatNumber(2, {format: "#,##0.00", locale: "br"})+'%');        
      
      } else if (Salario <= (9.5 * URP) && NovoRegulamento == 1){
      ...
      } else if (Salario > (9.5 * URP) || (Salario >= (9.5 * URP) && NovoRegulamento == 0)){
      
        $('#fnc17').show();
        $('#fnc2').show();        
                                
        $('#fnc3').val("R$ " + $.formatNumber(((9.5 * URP) * ($('#fnc1').val()/100)) + ((Salario -(9.5 * URP)) * ($('#fnc2').val()/100)), {format: "#,##0.00", locale: "br"}));
        ContribCalc[0][0] = ((9.5 * URP) * ($('#fnc1').val()/100)) + ((Salario -(9.5 * URP)) * ($('#fnc2').val()/100)) ;
        ContribCalc[0][1] = 0;
        ContribCalc[0][2] = 1;
        ContribCalc[0][3] = 0;
...
//------------------------------------------------------- Eventos Change --------------------------------------------------------- 
    ...
    $('#fnc1').change(function() {      
      ...
      if (isNaN($('#fnc1').val())) {
          ...
          $('#fnc1').val(0);                                                                                  
          ContribCalc[0][0] = ((9.5 * URP) * ($('#fnc1').val()/100)) + ((Salario -(9.5 * URP)) * ($('#fnc2').val()/100)) ;
          $('#fnc2').val(''+$.formatNumber($('#fnc2').val(), {format: "#,##0.00", locale: "br"})+'%');
          $('#fnc1').val("");
        ...
        if (Salario < (9.5 * URP) && NovoRegulamento == 0){
            ...
            if ($('#fnc1').val() > 12 || $('#fnc1').val() < 2) {
              $('#fnc27').html('<strong>Atenção!</strong> Para 1ª Faixa digite um valor entre 2% à 12%.');  
              $('#fnc27').fadeIn();
              $('#fnc1').val("");        
            ...
            if ($('#fnc1').val() > 3 || $('#fnc1').val() < 0) {
              $('#fnc27').html('<strong>Atenção!</strong> Para 1ª Faixa digite um valor entre 0% à 3%.');  
              $('#fnc27').fadeIn();
              $('#fnc1').val("");        
              $('#fnc3').val("R$ " + $.formatNumber(0, {format: "#,##0.00", locale: "br"})); 
              ContribCalc[0][0] = 0;                                 
    ...
    $('#fnc2').change(function() {          
      ...
      if (isNaN($('#fnc2').val())) {
        ...
        if (Salario > (9.5 * URP) || (Salario >= (9.5 * URP) && NovoRegulamento == 0)) {      
                
              if ($('#fnc2').val() > 13 || $('#fnc2').val() < 0) {
              $('#fnc27').html('<strong>Atenção!</strong> Para 2ª Faixa digite um valor entre 0% à 13%.');  
              $('#fnc27').fadeIn();
              $('#fnc2').val(0);                  
              ContribCalc[0][0] = ((9.5 * URP) * ($('#fnc1').val()/100)) + ((Salario -(9.5 * URP)) * ($('#fnc2').val()/100)) ;
              $('#fnc2').val("");
