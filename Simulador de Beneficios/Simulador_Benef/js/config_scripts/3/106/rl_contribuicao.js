...
//----------------------------------------------------- Inicializa Contribuições ---------------------------------------------------------  
    ...
    //Inicializa Contribuições
      ...
      } else if (Salario > (8 * URP)){
      
      $('#fnc17').show();
      $('#fnc2').show();
      $('#fnc2').prop("selectedIndex", 8);
    
      $('#fnc3').val("R$ " + $.formatNumber(((8 * URP) * $('#fnc1').val()) + ((Salario -(8 * URP)) * $('#fnc2').val()), {format: "#,##0.00", locale: "br"})); 
      ContribCalc[0][0] = ((8 * URP) * $('#fnc1').val()) + ((Salario -(8 * URP)) * $('#fnc2').val()) ;
      ContribCalc[0][1] = 0;
      ContribCalc[0][2] = 1;
      ContribCalc[0][3] = 0;             
     ...
     if(ContribCalc[0][0] * contrib_patroc > (0.085 * Salario)){ 
        $('#fnc10').val("R$ " + $.formatNumber(0.085 * Salario, {format: "#,##0.00", locale: "br"}));
        ContribCalc[1][0] = 0.085 * Salario;
        ContribCalc[1][1] = 1;
        ContribCalc[1][2] = 1;
        ContribCalc[1][3] = 0;
     } else {
...
function novoRegulamento() {
  ...
  if (DtAdesao <= new Date(DtEfetPlano)){   //Participante inscrito no plano até a data de aprovação do novo regulamento 17/02/1999 (data efetiva do novo regulamento) 
      NovoRegulamento = 0;  //Participante inscrito no plano até a data de aprovação do novo regulamento 17/02/1999
   } else {
      NovoRegulamento = 1;  //Participante inscrito no plano depois da data de aprovação do novo regulamento 17/02/1999
   }
