...
function calcBenefTC(){ //Benefício de aposentadoria por tempo de contribuição
 ...
 var AdesaoNaDtBase = (DtAdesao.getMonth() == DtSaldoIni.getMonth() && DtAdesao.getFullYear() == DtSaldoIni.getFullYear()) ? true : false;
 var PBC = AdesaoNaDtBase ? DtSaldoIni : ProxMes(DtSaldoIni, -1);  //Considera SRB no mes anterior a data base
 var FatAcumulado = 0;
 var i = 36;
  ...
  for(var l in vlSal){
     if (DataDif(vlSal[l][0], PBC, 0, 1) == 0 || HistSalario.length == 0){   //Encontra valores projetados até a data da aposentadoria
        ...
        for( var j = l; j > (l-QtdSalario); j--){//Soma 36 Salarios para cálculo do SRB
         ...
         INSSequalizado = vlSal[j][1] < vlSal[j][3] ? vlSal[j][1] : vlSal[j][3]; //INSS equalizado com o salario, quando salario < que inss
         INSSCorrigido = vlSal[j][1] == 0 ? INSSCorrigido : INSSCorrigido + FatAcumulado * INSSequalizado;
         ...
         vlSal[j][1] == 0 ? SalarioZero = true : ""; // flag de existência de Salario igual a 0
           ...
           if (isMobileApp()){
                TabelSRB = "<tr><td>" + i-- + "</td><td>R$ " + $.formatNumber(((FatAcumulado * (vlSal[j][1] > Teto ? Teto : vlSal[j][1])).toFixed(2) * 1), {format:"#,##0.00", locale:"br"}) + "</td><td>R$ " + $.formatNumber(((FatAcumulado * INSSequalizado).toFixed(2) * 1), {format:"#,##0.00", locale:"br"}) + "</td></tr>" + TabelSRB;
           } else {
                TabelSRB = "<tr><td>" + i-- + "</td><td>R$ " + $.formatNumber((vlSal[j][1] > Teto ? Teto : vlSal[j][1]), {format:"#,##0.00", locale:"br"}) + "</td><td>R$ " + $.formatNumber(INSSequalizado, {format:"#,##0.00", locale:"br"}) + "</td><td>" + FatAcumulado + "</td><td>R$ " + $.formatNumber(((FatAcumulado * (vlSal[j][1] > Teto ? Teto : vlSal[j][1])).toFixed(2) * 1), {format:"#,##0.00", locale:"br"}) + "</td><td>R$ " + $.formatNumber(((FatAcumulado * INSSequalizado).toFixed(2) * 1), {format:"#,##0.00", locale:"br"}) + "</td></tr>" + TabelSRB;
           }                   
        }
...
function calcBenefAuxDoenca(){ //Benefício de Auxilio Doenca
 ...
 var PBC = AdesaoNaDtBase ? DtSaldoIni : ProxMes(DtSaldoIni, -1);  //Considera SRB no mes anterior a data base
 var FatAcumulado = 0;
 var PBINSS = 0.91;   //% do beneficio INSS
 var i = 12;
 var GridSRB = "";
 var TabelSRB = "";
 var QtdSalario = 0;  //Quantidade de salário a considerar para cálculo
 ...
 $("#secValidacaoTemporario_msg1").html("");
  ...
  for(var l in vlSal){
     if (DataDif(vlSal[l][0], PBC, 0, 1) == 0 || HistSalario.length == 0){   //Encontra valores projetados até a data da aposentadoria
        //QtdSalario = (l * 1) + 1, aplicado este tratamento para contagem correta dos salários a serem considerados quando quantidade menor que 12 
         l < 12 ? (l == 0 ? QtdSalario = 1 : QtdSalario = (l * 1) + 1) : QtdSalario = 12; //Limitador de salários a considerar
         i = QtdSalario;
        for( var j = l; j > (l-QtdSalario); j--){//Soma 12 Salarios para cálculo do SRB
         var Teto = 4 * (vlSal[j][3] != 0 ? vlSal[j][3] : Teto_INSS);
         FatAcumulado = acumula_indicador(vlINPC, vlINPC[j][0], vlINPC[l][0]);
         SRB = vlSal[j][1] == 0 ? SRB : SRB + FatAcumulado * (vlSal[j][1] > Teto ? Teto : vlSal[j][1]);
         INSSequalizado = vlSal[j][1] < vlSal[j][3] ? vlSal[j][1] : vlSal[j][3]; //INSS equalizado com o salario, quando salario < que inss         
  ...
  $("#secValidacaoTemporario_msg1").html("<strong>Salários Considerados para Efeito de Cálculo do SRB</strong><br/><br/>" + GridSRB);

  SRBMedia = SRB/QtdSalario;
  INSSCorrigido = INSSCorrigido/QtdSalario;

  //Dedução % do beneficio INSS
  INSSCorrigido = INSSCorrigido * PBINSS;

  SRB = SRBMedia > TetoSalPart ? TetoSalPart : SRBMedia;

  //Alert de Histórico de Salário igual a 0
  if (SalarioZero &&  $("#secTemporario").html().indexOf("secValidacaoTemporario_alertMsg1")<0) {
     $("#secTemporario").html('<div class="alert alert-danger"><span id="secValidacaoTemporario_alertMsg1"><strong>ATENÇÃO!</strong> Em seu histórico de salários existem registros com valor igual a R$0,00.<br /> Por gentileza entre em contato com a FIPECq, para maiores esclarecimentos.</span></div>' + $("#secTemporario").html());
  }

  //Monta Div's com ID
  $("#secValidacaoTemporario_msg1").html($("#secValidacaoTemporario_msg1").html() + '<div id="secTempVariaveisDesc"></div><div id="secTempDemonstraCalc"></div>');

  //Alimentação inicial variáveis descrição
  VariaveisDesc = '<p align="center"><strong>Valores comparativos para cálculo do Auxílio Doença</strong></p><br/><div class="row" align="left"><div class="col-md-4 col-sm-4">' + secValidacaoINSS + '<strong> INSS: </strong>R$ '+ $.formatNumber((INSSCorrigido.toFixed(2) * 1) , {format:"#,##0.00", locale:"br"}) + ' </div> <div class="col-md-4 col-sm-4">' + secValidacaoSRB + '<strong> SRB: </strong>R$ ' +  $.formatNumber((SRBMedia.toFixed(2) * 1) , {format:"#,##0.00", locale:"br"})  +' </div>';  

  Benef = SRB - INSSCorrigido;

  DemonstraCalc = DemonstraCalc + '<p align="left">SRB - INSS = ' + $.formatNumber((Benef.toFixed(2) * 1) <= 0 ? 0 : (Benef.toFixed(2) * 1), {format:"#,##0.00", locale:"br"}) +' (Total)</p>';

  if (Benef >= (SRB * 0.20)){     
     Benef = Benef;
     DemonstraCalc = DemonstraCalc + '<br/><p align="left"><strong>Benefício Bruto Calculado</strong></p><br /><p align="left">Auxílio Doença: <span class="valReceita">R$ ' + $.formatNumber(((Benef.toFixed(2) * 1) <= 0 ? 0 : (Benef.toFixed(2) * 1)), {format:"#,##0.00", locale:"br"}) +'</span></p>'; 
  } else {
     Benef = SRB * 0.20;
     DemonstraCalc = DemonstraCalc + '<br/><p align="left"><strong>Benefício Bruto Calculado</strong></p><br /><p align="left">Auxílio Doença Teto: <span class="valReceita">R$ ' + $.formatNumber((Benef.toFixed(2) * 1) , {format:"#,##0.00", locale:"br"}) +'</span></p>';
  }
  ...
  //Conteúdo de apresentação Div secTemporario_info_flex  
  HtmlInfoFlex = '<div class="panel panel-default">' +
                '<div class="panel-body">' +
                '<div class="row">' +
                '<div class="col-xs-0 col-sm-1 col-md-1"></div>' +
                '<div class="col-xs-12 col-sm-10 col-md-10">' +
                '<span>Veja abaixo o valor de seu benefício complementar futuro</span>' +
                '</div>' +
                '<div class="col-xs-0 col-sm-1 col-md-1"></div>' +
                '</div>' +
                '<div class="row">&nbsp;</div>' +
                ...
                '<div class="col-xs-0 col-sm-1 col-md-1"></div>' +                
                '<div class="col-xs-12 col-sm-10 col-md-10">' +
                '<p><strong>Auxílio Doença: </strong><span class="valReceita">R$ ' + $.formatNumber(Benef, {format: "#,##0.00", locale: "br"}) + '</span></p>' +
                '</div>' +
                '<div class="col-xs-0 col-sm-1 col-md-1"></div>' +                
                '</div>' +
                '<div class="row">' +
                ...
                '</div>' +
                '<div class="row">' +
                '<div class="col-xs-12 col-sm-12 col-md-12 center hidden-xs hide">' +
  ...
  //if(!isMobileApp()) {
     //Imprime o HTML das DIVs
     $("#secTempVariaveisDesc").html(VariaveisDesc);
     $("#secTempDemonstraCalc").html(DemonstraCalc);              
     $("#secTemporario_info_flex").html(HtmlInfoFlex);
     $("#secTemporario_info_generico").hide();    
     $("#secTemporario_info_flex").show();    
  //}
