...
function Monta_Contrib(){
    ...
    var ctr1fd = new tbfld("Normal Participante", fnc1, fnc2, fnc3, fnc4);

    //config grupo contribuição 2
    var ctr2hd = new tbfld("", "", "", "", "Contrib. Simulada");
    var ctr2fd = new tbfld("Normal Patrocinadora", "", "", "", fnc8);
...
function checkJoia(){  //Retorna Status de pagamento Contrib Joia participante             
  ...
  do {
    ...
    if (movto.id_conta == 1205 && movto.conta_resp == 0 && ctrCatch == 0){ //Pega a última Contrib Jóia
      contribJoia =  movto.ctr_rent;
      ctrCatch++;       
    }
    if (movto.id_conta == 1205 && movto.conta_resp == 0){ //Se Conta Jóia e Responsável Participante = True
      contaJoia++;
    }
    i--;                                      
  } while(i >= 0)
...
function calcContribJoia(saida){ //Saida = 0 - Pagamento a prazo, Saida = 1 - Pagamento a vista
   ...
   for(var i in Tabua){ 
     var obj = Tabua[i]; 
     var obj2 = Tabua[parseInt(i)+1] == undefined ? Tabua[i] : Tabua[parseInt(i)+1] ;
