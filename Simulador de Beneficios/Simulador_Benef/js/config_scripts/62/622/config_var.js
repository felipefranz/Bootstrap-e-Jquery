...
var MaxPerc = 2;                                  //Valor máximo da forma de pagamento Percentual
...
var TabuaBmBpd = [];                              //Array Tabua Atuarial - BM BPD
//var CalcBenefMin = 0;                           //Valor de benefício mínimo calculado na engine
var SaldoMin = URP * 100;                      		        //Valor do saldo mínimo para pagamento único
var DtLimitAdesao = new Date("03/31/2014");                    //Data de comparação com a data efetiva do participante no plano GebsaPrev
...
var FlagContribBasica = DadosPlano.TipoClassificacao == '11' ? true : false;
var ElegBM = DadosPlano.ElegBM && status != 9? true : false;    //Elegibilidade a BM - Benefício Mínimo
var SalarioDesligado = DadosPlano.SalarioDesligado != null ? DadosPlano.SalarioDesligado : 0;    //Salário no desligamento
var Salario1_BM = DadosPlano.Salario1_BM != null ? DadosPlano.Salario1_BM : 0;    //Salário 1 cálculo BM
...
var alterBeneficio = status == 9 ? true: false; // Verifica se o participante é assistido
var dtAlteracaoFormaRecebimentoBenef = DadosPlano.DtAltFormaRecebimentoAssistido; //Inclusão de informações de saque e alteração de benefício
var InfoSaqueBenefAssistido = DadosPlano.InfoSaqueAssistido; //Inclusão de informações de saque e alteração de benefício
var limiteQtdeSaque = 5; //Limite de quantidades de saques que um participante pode realizar

// IncSaque = 0.1  //Habilitar Percentual de saque com incremente de 0,1 em 0,1 para todos status; 
...
//Alterações saque assistido
var secBenefAnterior_dtAltFormRecebimento = "Última Alteração da Forma de Recebimento: ";
var secBenefAnterior_qtdSaque = "Quantidade de Saques Efetuados:";
var secBenefAnterior_percentSaque = "Percentual Sacado:";
//var secBenefAnterior_valSaque = "Valor Total Sacado:"; //Excluido
//var secBenefAnterior_percentSaldoRemanescente = "Percentual Remanescente:"; //Excluido
var divDtAlteracaoFormaRecebimento = '<div class="col-xs-12 col-sm-12 col-md-12"><label for="DtAltFormRecebimento"><span id="secBenefAnterior_dtAltFormRecebimento"></span></label><input type="text" id="DtAltFormRecebimento" class="valReceita" readonly></div>';
var divQtdSaque = '<div class="col-xs-12 col-sm-12 col-md-12"><label for="QtdSaque"><span id="secBenefAnterior_qtdSaque"></span></label><input type="text" id="QtdSaque" class="valReceita" readonly></div>';
var divPercentSaque = '<div class="col-xs-12 col-sm-12 col-md-12"><label for="PercentSaque"><span id="secBenefAnterior_percentSaque"></span></label><input type="text" id="PercentSaque" class="valReceita" readonly></div>';
//var divValSaque = '<div class="col-xs-12 col-sm-12 col-md-12"><label for="ValSaque"><span id="secBenefAnterior_valSaque"></span></label><input type="text" id="ValSaque" class="valReceita" readonly></div>'; //Excluido
//var divPercentSaldoRemanescente = '<div class="col-xs-12 col-sm-12 col-md-12"><label for="PercentSaldoRemanescente"><span id="secBenefAnterior_percentSaldoRemanescente"></span></label><input type="text" id="PercentSaldoRemanescente" class="valReceita" readonly></div>'; //Excluido
var secSaque_DocSaque = '<div class="col-xs-12 col-sm-12 col-md-12"><span id="spanDocSaque" style="display: inline;"><p align="justify"><hr>Para formalizar a alteração <a href="https://www.gebsaprev.org.br/download/Documentos_do_Plano/Formularios/GEBSAPrev_ASSISTIDO_%20RETIRADA_SALDO_DE_CONTA.pdf" target="_blank"><b>clique aqui</b></a> e imprima o formulário "Requerimento de Retirada Percentual do Saldo de Conta", assine, reconheça firma e entregue em uma de nossas unidades de atendimento.</p></span></div>';

//Formularios
//var frmContribExtra = '<form id="frmParametros" method="post" action="https://www.portal-hro.com.br/cadastro/LoginForm.aspx" target="_blank"> <!-- generico para esportal --> <input type="hidden" name="userName"       value="0062ua.application"> <input type="hidden" name="passWord"       value="null"> <input type="hidden" name="urlGo"          value=""/> <input type="hidden" name="menuOff"        value="True"> <input type="hidden" name="showBackButton" value="False"> <input type="hidden" name="idParticipante" value="null"></form>';
var frmContribExtra = '<form id="frmParametros" method="post" action="https://www.portal-hro.com.br/cadastro/LoginForm.aspx" target="_blank"> <!-- generico para esportal --> <input type="hidden" name="userName"       value="0062' + CPF + '"> <input type="hidden" name="passWord"       value="null"> <input type="hidden" name="urlGo"          value=""/> <input type="hidden" name="menuOff"        value="True"> <input type="hidden" name="showBackButton" value="False"> <input type="hidden" name="idParticipante" value="null"></form>';
var secAltBenef = '<div id="secAltBenef" class="panel panel-default hidden-xs"><div class="panel-body"><span id="secAltBenef_msg1"><p align="justify"></p><p>Para formalizar a alteração, imprima o formulário "Requerimento de Alteração da Forma de Recebimento de Benefício de Aposentadoria", assine, reconheça firma e entregue em uma de nossas unidades de atendimento.</p><p align="center" style="margin-top: 20px;"><a type="button" id="btnCampAlterBeneficio" href="javascript:void(0)" onclick="GerarForm(2)" target="_blank" class="btn btn-primary">Gerar Formulário</a></p></span></div></div>';
...
for (var l in DadosPlano.Tabua){
        ...
        TabuaBmBpd.push({ idade: parseInt(obj.idade), fator_fem: parseFloat(obj.fator_fem), fator_masc: parseFloat(obj.fator_masc)});
    };
