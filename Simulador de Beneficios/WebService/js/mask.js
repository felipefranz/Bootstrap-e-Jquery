void function delField(campo, evt) {

    if (evt.keyCode == 46 || evt.keyCode == 8) {
        campo.value = "__.___.___/____-__";

        if (campo.createTextRange) {
            var posicao = campo.createTextRange();
            posicao.move("character", 0);
            posicao.select();
...
function isNumberDecimal(campo, evt) {
...
function strReplaceChr(palavra) {
    com_acento = '.-_@áàãâäéèêëíìîïóòõôöúùûüÁÀÃÂÄÉÈÊËÍÌÎÏÓÒÕÖÔÚÙÛÜ';
    sem_acento = '.-_@aaaaaeeeeiiiiooooouuuuAAAAAEEEEIIIIOOOOOUUUU';
    nova = '';
...
function removeDecimal(campo)
{
    if(campo.value.indexOf(',') != -1)campo.value = campo.value.substr(0,campo.value.indexOf(','));
}